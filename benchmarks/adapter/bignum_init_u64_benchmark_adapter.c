/**
 * @file bignum_init_u64_benchmark_adapter.c
 * @brief Deterministic benchmark-framework callbacks for bignum_init_u64.
...
typedef struct bignum_init_u64_benchmark_state {
    bignum_t a;
    bignum_t b;
    bignum_t result;
    uint64_t words[2];
...
} bignum_init_u64_benchmark_state_t;
...
{
    if (workload == NULL) return BIGNUM_INIT_U64_BENCHMARK_STATUS_NULL_ARGUMENT;
    if (workload->operation_kind == NULL || workload->size_profile == NULL ||
        workload->capacity_profile == NULL) return BIGNUM_INIT_U64_BENCHMARK_STATUS_INVALID_PROFILE;
    if (strcmp(workload->operation_kind, "init_u64") != 0 &&
        strcmp(workload->operation_kind, "mixed") != 0) return BIGNUM_INIT_U64_BENCHMARK_STATUS_INVALID_PROFILE;
    return BIGNUM_INIT_U64_BENCHMARK_STATUS_SUCCESS;
}
...
{
    bignum_init_u64_benchmark_state_t *state = opaque;
    (void)context;
    if (state == NULL || bignum_init_u64_benchmark_validate_workload(workload) != BIGNUM_INIT_U64_BENCHMARK_STATUS_SUCCESS)
    ...
    const benchmark_workload_t *workload, void *context)
{
    ...
    rc = (int)(bignum_init_u64(&state->result, UINT64_C(3)));
    if (strcmp("init_u64", "is_zero") == 0) {
...
{
    const bignum_init_u64_benchmark_state_t *state = opaque;
...
bignum_init_u64_benchmark_status_t bignum_init_u64_benchmark_adapter_init(benchmark_adapter_t *adapter)
{
    if (adapter == NULL) return BIGNUM_INIT_U64_BENCHMARK_STATUS_NULL_ARGUMENT;
    *adapter = (benchmark_adapter_t){
        .benchmark_name = "bignum_init_u64", .state_size = sizeof(bignum_init_u64_benchmark_state_t),
